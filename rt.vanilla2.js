...
const Vector = {
    ...
    cross: (v1, v2) => vector(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x)
};
...
const Color = {
    ...
    toDrawingColor: ({ r, g, b }) => color(floor(min(r, 1) * 255), floor(min(g, 1) * 255), floor(min(b, 1) * 255))
};
...
class RayTracer {
    ...
    shade({ thing, dist, ray: { start, dir } }, scene, depth) {
        ...
        return Color.plus(Color.plus(Color.background, this.getNaturalColor(thing, pos, normal, reflectDir, scene)), (depth >= this.maxDepth) ? Color.grey : this.getReflectionColor(thing, pos, normal, reflectDir, scene, depth));
    }
    ...
    render(scene, ctx, screenWidth, screenHeight) {
        ...
        const getPoint = (x, y, camera) => Vector.norm(Vector.plus(camera.forward, Vector.plus(Vector.times(recenterX(x), camera.right), Vector.times(recenterY(y), camera.up))));
        const { camera } = scene;
        for (let y = 0; y < screenHeight; y++) {
            for (let x = 0; x < screenWidth; x++) {
                const { r, g, b } = Color.toDrawingColor(this.traceRay(ray(camera.pos, getPoint(x, y, camera)), scene, 0));
